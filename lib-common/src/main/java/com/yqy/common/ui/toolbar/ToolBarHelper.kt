package com.yqy.common.ui.toolbar
...
class ToolBarHelper(private val mContext: Activity, layoutId: Int, private val navigationIconRes: Int, private val toolBarBackgroundRes: Any?) {
    ...
    init {
        toolbarHeight = BarUtils.getStatusBarHeight()
        /*初始化整个内容*/
        initContentView()
        /*初始化用户定义的布局*/
    ...
    private fun initUserView(id: Int) {
        ...
        val toolBarSize = typedArray.getDimension(1, mContext.resources.getDimension(R.dimen.toolbar_height).toInt().toFloat()).toInt() + if (Build.VERSION.SDK_INT >= 21) toolbarHeight else 0
        typedArray.recycle()
        /*如果是悬浮状态，则不需要设置间距*/
